...
const fetchDataFromFakeApi = size => {

    const FETCH_URL = `https://api.instantwebtools.net/v1/passenger?page=0&size=${size}`;
    ...
    return fetch(FETCH_URL, {
        ...
        .then(d => {
            const convertedData = [];
            d.data.forEach(passenger => convertedData.push(convertPassengerObj(passenger)));
            return convertedData;
        });
};

const convertPassengerObj = obj => {
    let result = {};
 ...
 */
const asyncAPIService = async dataSize => {
    const totalData = await fetchDataFromFakeApi(dataSize);
